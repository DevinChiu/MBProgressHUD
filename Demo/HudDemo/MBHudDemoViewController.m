...
- (void)indeterminateExample {
    ...
    MSMBProgressHUD *hud = [MSMBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
...
- (void)windowExample {
    ...
    MSMBProgressHUD *hud = [MSMBProgressHUD showHUDAddedTo:self.view.window animated:YES];
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        [self doSomeWork];
        dispatch_async(dispatch_get_main_queue(), ^{
            [hud hideAnimated:YES];
...
- (void)customViewExample {
    ...
    hud.mode = MSMBProgressHUDModeCustomView;
...
- (void)textExample {
    ...
    hud.mode = MSMBProgressHUDModeText;
    hud.label.text = NSLocalizedString(@"Message here!", @"HUD message title");
    // Move to bottm center.
    hud.offset = CGPointMake(0.f, MSMBProgressMaxOffset);
...
- (void)dimBackgroundExample {
    ...
    hud.backgroundView.style = MSMBProgressHUDBackgroundStyleSolidColor;
...
- (void)doSomeWorkWithProgress {
    ...
    while (progress < 1.0f) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [MSMBProgressHUD HUDForView:self.navigationController.view].progress = progress;
        });
        usleep(50000);
    }
}

- (void)doSomeWorkWithMixedProgress:(MSMBProgressHUD *)hud {
    // Indeterminate mode
    sleep(2);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        hud.mode = MSMBProgressHUDModeIndeterminate;
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        MSMBProgressHUD *hud = [MSMBProgressHUD HUDForView:self.navigationController.view];
        UIImage *image = [[UIImage imageNamed:@"Checkmark"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
        hud.customView = imageView;
        hud.mode = MSMBProgressHUDModeCustomView;
